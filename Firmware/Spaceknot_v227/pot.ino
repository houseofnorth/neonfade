...
// Flush the rolling average buffer with fresh readings.
// Call before capturing min/max so stale motion samples don't skew the result.
static void _flushPotAvg() {
  for (int i = 0; i < POT_AVG_N; i++) {
...
void saveCalibration() {
  ...
  _calibrating = false;
  _autoSweep   = false;
  LOG1("Cal saved: min=%d max=%d range=%d. Motor enabled.\n",
...
// ── MQTT-driven setters ───────────────────────────────────────────────────
//  Called by the MQTT callback when another unit broadcasts its captured value.
//  We set the value directly so all units end up with identical ADC numbers,
//  rather than each reading its own (possibly different) pot position.

void setCalMin(int v) {
  if (!_calibrating) return;
  _potMin = v;
  LOG2("Min set from MQTT: %d\n", v);
}

void setCalMax(int v) {
  if (!_calibrating) return;
  _potMax = v;
  LOG2("Max set from MQTT: %d\n", v);
}

// Apply a state position received from another unit; saves to NVS immediately.
void setStatePosDirect(DimState s, int v) {
  if (s < DIM_DAY || s > DIM_NIGHT) return;
  static const char* keys[] = { PREFS_KEY_DAY, PREFS_KEY_DUSK, PREFS_KEY_NIGHT };
  _statePos[s] = v;
  _prefs.putInt(keys[s], v);
  LOG2("State pos [%d] set from MQTT: %d\n", (int)s, v);
}

// ── Direct range application (slider UI or MQTT setrange) ─────────────────
//  Validates, saves to NVS, and exits cal mode (if active).
void applyCalibrationRange(int newMin, int newMax) {
  const int range = newMax - newMin;
  if (range < CALIB_MIN_RANGE) {
    LOG1("ERR: range too small (%d) — min=%d max=%d\n", range, newMin, newMax);
    return;
  }
  _potMin      = newMin;
  _potMax      = newMax;
  ...
  _calibrating = false;
  _autoSweep   = false;
  LOG1("Range applied: min=%d max=%d range=%d\n", _potMin, _potMax, range);
}
