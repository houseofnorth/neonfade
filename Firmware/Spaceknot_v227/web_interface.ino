...
static void _onCal() {
  ...
  else if (act == "min")    { captureMin();                mqttPublishCal(("min:"    + String(getPotMin())).c_str());            }
  else if (act == "max")    { captureMax();                mqttPublishCal(("max:"    + String(getPotMax())).c_str());            }
  ...
  else if (act == "day")    { captureStatePos(DIM_DAY);   mqttPublishCal(("capDay:"   + String(getStatePos(DIM_DAY))).c_str());   }
  else if (act == "dusk")   { captureStatePos(DIM_DUSK);  mqttPublishCal(("capDusk:"  + String(getStatePos(DIM_DUSK))).c_str());  }
  else if (act == "night")  { captureStatePos(DIM_NIGHT); mqttPublishCal(("capNight:" + String(getStatePos(DIM_NIGHT))).c_str()); }
  else if (act == "motorfwd")  { calDriveMotor(FORWARD);  }
  else if (act == "motorrev")  { calDriveMotor(REVERSE);  }
  else if (act == "motorstop") { calDriveMotor(STOPPED);  }
  else if (act == "setrange") {
    int newMin = _server.arg("min").toInt();
    int newMax = _server.arg("max").toInt();
    applyCalibrationRange(newMin, newMax);
    mqttPublishCal(("setrange:" + String(newMin) + ":" + String(newMax)).c_str());
  }
