...
static bool     _calMotorActive = false;   // motor explicitly driven during cal mode
...
static void _updateDiscreteMode() {
  ...
  if (_settled) {
    ...
    if (dist > MANUAL_TURN_THRESHOLD) {
      ...
      LOG2("Manual pot turn — motor frozen\n");
      return;
    }
...
// Main motor update.
// Motor is stopped if: range not calibrated, relay OFF, or cal mode with no drive request.
void updateMotor() {
  if (isCalibrating()) {
    if (!_calMotorActive) _motorStop();  // safe default; calDriveMotor() overrides
    return;
  }
  _calMotorActive = false;  // reset when leaving cal
  if (!calibDone()) { _motorStop(); return; }
  if (!getRelayState()) { _motorStop(); return; }
  _updateDiscreteMode();
}
...
void holdMotorPosition() {
  ...
  ledcWrite(MOTOR_IN2, 0);
  _motorDir       = STOPPED;
  _targetPotPos   = readPot();
  _settled        = true;
  _manualOverride = false;
}

// Drive the motor directly during calibration.
// FORWARD = higher ADC (more resistance); REVERSE = lower ADC.
// Calling with STOPPED cuts PWM immediately.
void calDriveMotor(MotorDir dir) {
  if (!isCalibrating()) return;
  if (dir == FORWARD)      { _driveForward();  _calMotorActive = true; }
  else if (dir == REVERSE) { _driveReverse();  _calMotorActive = true; }
  else                     { _motorStop();     _calMotorActive = false; }
}
