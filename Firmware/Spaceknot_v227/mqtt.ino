...
static void _mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  //  min/max/capXxx carry the master unit's actual ADC value so every unit
  //  ends up with identical calibration numbers instead of reading its own pot.
  //  Formats: "min:NNN"  "max:NNN"  "capDay:NNN"  "setrange:NNN:MMM"
  if (String(topic) == MQTT_TOPIC_CAL) {
    if      (msg == "start")              enterCalibration();
    else if (msg.startsWith("min:"))      setCalMin(msg.substring(4).toInt());
    else if (msg.startsWith("max:"))      setCalMax(msg.substring(4).toInt());
    else if (msg == "auto")               toggleAutoSweep();
    else if (msg == "save")               saveCalibration();
    else if (msg == "cancel")             cancelCalibration();
    else if (msg.startsWith("capDay:"))   setStatePosDirect(DIM_DAY,   msg.substring(7).toInt());
    else if (msg.startsWith("capDusk:"))  setStatePosDirect(DIM_DUSK,  msg.substring(8).toInt());
    else if (msg.startsWith("capNight:")) setStatePosDirect(DIM_NIGHT, msg.substring(9).toInt());
    else if (msg.startsWith("setrange:")) {
      int c1 = msg.indexOf(':', 9);
      if (c1 >= 0) applyCalibrationRange(msg.substring(9, c1).toInt(), msg.substring(c1 + 1).toInt());
    }
    return;
  }
