...
.t1{font-size:2rem;font-weight:900;letter-spacing:.45em;text-transform:uppercase;margin-bottom:.35rem}
...
<div class="title">
  <div class="t2">K E R I M  &nbsp; S E I L E R</div>
...
<div id="rawAdc" style="text-align:center;font-size:.75rem;color:#555;letter-spacing:.08em;margin-bottom:1rem;display:none"></div>
...
<small>Drive to each hard stop, capture, then save.</small><br><br>
<div class="cal-grid" style="margin-bottom:.4rem">
<button class="btn" onmousedown="cal('motorfwd')" onmouseup="cal('motorstop')" ontouchstart="cal('motorfwd')" ontouchend="cal('motorstop')">&#9650; Drive FWD</button>
<button class="btn" onmousedown="cal('motorrev')" onmouseup="cal('motorstop')" ontouchstart="cal('motorrev')" ontouchend="cal('motorstop')">&#9660; Drive REV</button>
</div>
<div class="cal-grid">
...
<div class="field">
  <label>Firmware update — upload .bin file</label>
  <input type="file" id="fwFile" accept=".bin">
</div>
<button class="btn" onclick="uploadFirmware()">Flash firmware</button>
...
async function poll(){
  try{
    ...
    // Bar — show raw ADC position during calibration, calibrated % otherwise
    const barPct = d.calibrating ? Math.round(d.pot/4095*100) : d.brightnessPct;
    $('bmin').textContent = d.calibrating ? '0'    : d.potMin;
    $('bmax').textContent = d.calibrating ? '4095' : d.potMax;
    $('bfill').style.width = barPct+'%';
    const raw=$('rawAdc');
    raw.style.display=d.calibrating?'block':'none';
    if(d.calibrating) raw.textContent='ADC '+d.pot;
...
async function uploadFirmware(){
  ...
  msg.textContent='Uploading… do not close this page.';
  const fd = new FormData();
  fd.append('firmware', file);
  try {
    const r = await fetch('/update', {method:'POST', body:fd});
    if(r.ok){ msg.textContent='Done — unit is rebooting.'; }
    else    { msg.textContent='Upload failed ('+r.status+').'; }
  } catch(e) {
    msg.textContent='Unit rebooted — update likely OK.';
  }
}
</script>
