...
void initWifi() {
  ...
  if (WIFI_MODE_CHOICE == SK_WIFI_AP) _startAp();
  else                                _startSta();
}
  ...
  switch (_state) {
    case WF_STA_TRYING:
      if (WiFi.status() == WL_CONNECTED) {
        ...
        LOG2("[WiFi] connected  %s\n", _ipString.c_str());
        _enter(WF_STA_UP);
      } else if (millis() - _stateEnteredAt > WIFI_CONNECT_TIMEOUT_MS) {
        LOG2("[WiFi] connect timeout\n");
        if (WIFI_MODE_CHOICE == SK_WIFI_DUAL) {
